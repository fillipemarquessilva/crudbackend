...
const generateToken = (usuario) => {
  const token = jwt.sign({ usuario, exp: Math.floor(Date.now() / 1000) + 120 }, jwtSecret); // Token expira em 1 minuto
  return token;
};
...
app.post('/api/login', (req, res) => {
  try {
    ...
    if (!usuarioAutenticado) {
      return res.status(400).json({ error: 'Credenciais inválidas abestado.' });
    }
